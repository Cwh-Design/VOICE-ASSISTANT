...
void main() {
  ...
  // 垂直方向渐隐：让声波纹的上下边缘平滑融入背景，避免和后面的图层产生硬切
  float vFade = 1.0 - smoothstep(0.18, 0.33, abs(uv.y));
  col *= vFade;
...
void main() {
  ...
  // 垂直方向渐隐：玻璃球上下边缘平滑融入背景
  float vFade = 1.0 - smoothstep(0.32, 0.48, abs(p.y));
  outRGB *= vFade;
  outA *= vFade;
...
/** Strands：WebGL 声波纹主视觉，来自 react-bits */
